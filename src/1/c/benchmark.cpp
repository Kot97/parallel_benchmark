...
int main(int argc, char const *argv[])
{
    ...
    unsigned long *values = (unsigned long*)malloc(sizeof(unsigned long)*size);
    ...
    unsigned long *temp_values= values;
    ...
    free(values); free(times_pthread); free(times_openmp); free(times_openmp_for); free(times_openmp_for_simd);
    return 0;
}
