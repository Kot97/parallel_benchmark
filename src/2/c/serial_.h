...
long dictionary[DCACHE_SIZE] = { 0 };

long c2_serial_rec_dict(long n)
{
    if (n <= 2)
        return 1;
    if (n < DCACHE_SIZE) {
        if (dictionary[n] != 0)
            ...
            return dictionary[n] = c2_serial_rec_dict(n - 1) + c2_serial_rec_dict(n - 2);
    } else {
        return c2_serial_rec_dict(n - 1) + c2_serial_rec_dict(n - 2);
    }
}
