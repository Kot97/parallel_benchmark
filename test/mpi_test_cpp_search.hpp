...
INSTANTIATE_TEST_SUITE_P(Small, MpiSearchTest, ::testing::Values(
...
INSTANTIATE_TEST_SUITE_P(Medium, MpiSearchTest, ::testing::Values(
...
INSTANTIATE_TEST_SUITE_P(Large, MpiSearchTest, ::testing::Values(
...
#define MPI_TEST_SEARCH(test_name, function_name)\
TEST_P(MpiSearchTest, test_name)\
{\
    double *max = (double*)malloc(sizeof(double));\
    function_name(comm, max, a, b);\
    if(comm.rank() == 0) ASSERT_DOUBLE_EQ(*max, GetParam().second);\
    free(max);\
}
